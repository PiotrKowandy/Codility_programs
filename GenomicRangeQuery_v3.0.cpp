...
int which_min (std::string &S, int start, int end){

	if (decod(S[start]) != 1 || decod(S[end]) != 1){	
		int min=4;
		for (std::string::iterator it=S.begin()+start; it<S.begin()+end+1; it++){
			if (decod(*it) < min) min = decod(*it);
			if (decod(*it) == 1) return 1;
		}
		return min;
	}
	else{
...
std::vector<int> solution (std::string &S, std::vector<int> &P, std::vector<int> &Q){
	...
	for (int i=0; i<length_P; i++) result[i] = which_min(S, P[i], Q[i]);
	return result;
}
