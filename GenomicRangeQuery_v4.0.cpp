...
std::vector<int> solution (std::string &S, std::vector<int> &P, std::vector<int> &Q){
	const int max = 4;
	int length_P = P.size();
	std::vector<int> result(length_P, max); 
	int length_S = S.size();

	for (int position=0; position < length_S; position++){
		for (int i=0; i < length_P; i++) {
			if (position >= P[i] && position <= Q[i]){
				if (decod(S[position]) < result[i]) result[i] = decod(S[position]);
			}
		}
	}
	return result;
